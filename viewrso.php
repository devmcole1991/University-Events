...
$query = $user_home->runQuery("SELECT R.* FROM rso R, joins J WHERE J.RSOId = R.RSOId AND J.userID = $userID");
        ...
        <nav class="navbar navbar-transparent navbar-absolute">
            ...
            while ($row2 = $query->fetch(PDO::FETCH_ASSOC)) {
                ...
                if(isset($_POST[$button1]))
                {
                    ...
                    if($user_home->leaveRSO($userID, $RSOId))
                    {
                        $user_home->updateRSONumber($RSOId, $membersNo);
                        echo " yay you did it";
                        $user_home->redirect('viewrso.php');
                    }
                    else
                    {
                        die("fml you failed");
                ...
                <div class="col-lg-6 col-md-12">
                    <div class="card">
                        ...
                        <div class="card-content table-responsive">
                            ...
                            <div>
                                <?php
                                    $query3 = $user_home->runQuery("SELECT * FROM makes M WHERE M.RSOId = $RSOId AND M.userID = $userID");
                                ...
                                <form action="" method="POST">
                                    <button class="btn btn-medium btn-info" type="submit" name="btn-leave<?php echo $i; ?>" style="text-align:right">Leave
                                </form>
                                <?php } ?>
                            </div>
                        </div>
