...
$query = $user_home->runQuery("SELECT * FROM event E, creates C, universityprofile UP
                                    WHERE (E.privacy = 'public' OR E.privacy = 'private') AND C.userID = $userID AND C.name = UP.name AND C.name = E.uName
                                    AND NOT EXISTS (SELECT * FROM approvespriv Pr WHERE E.eventId = Pr.eventId) 
                                    AND NOT EXISTS (SELECT * FROM approvespub Pu WHERE E.eventId = Pu.eventId)");
    ...
    <div class="main-panel" style="background-image: url('assets/img/wallpaper.jpeg'); background-size: cover; background-position: top center; min-height: 600px;">
        <nav class="navbar navbar-transparent navbar-absolute">
            ...
            while ($row2 = $query->fetch(PDO::FETCH_ASSOC)) {
                $button1 = "btn-approve".$i;
                $button2 = "btn-decline".$i;
                ...
                if(isset($_POST[$button1]))
                {
                    if($privacy == "public")
                    {
                        if($user_home->approvePublicEvent($eventId, $userID))
                        {
                            echo " yay you did it";
                            $user_home->redirect('approveevent.php');
                        }
                        else
                        {
                            die("fml you failed");
                    ...
                    {
                        if ($user_home->approvePrivateEvent($eventId, $userID))
                        {
                            echo " yay you did it";
                            $user_home->redirect('approveevent.php');
                        }
                ...
                {
                    if($user_home->deleteEvent($eventId))
                    {
                        echo " yay you did it";
                        $user_home->redirect('approveevent.php');
                    }
                ...
                ?>
                        <div class="col-md-4">
                            ...
                            <div>
                                ...
                                <button class="btn btn-medium btn-info" type="submit" name="btn-approve<?php echo $i; ?>" style="text-align:right">Approve
                                <button class="btn btn-medium btn-danger" type="submit" name="btn-decline<?php echo $i; ?>">Decline
                                </form>
                            </div>
                                    </div>
                                </div>
